    ...
    public partial class Form1 : Form
    {
        ...
        private void LabelSeat_Click(object sender, EventArgs e)
        {
            if (isEditorMode)
            {
                ...
                Seat seat2 = movie.SeatList.SearchByRowAndColumn(seatInfo2.Row, seatInfo2.Column);
                if (seat2.BookStatus)
                {
                ...
                seat2.CanBook = enableSeats.Checked;
                if (seat2.CanBook)
                {
            ...
            if (seat.BookStatus == false)
            {
                ...
                if (seatSelected > 0)
                {
                    ...
                    if (leftSeat != null)
                    {
                        ...
                        else if (leftSeat.BookByPerson != "" && movie.isSafeDistance)
                        {
                    ...
                    if (rightSeat != null)
                    {
                        ...
                        else if (rightSeat.BookByPerson != "" && movie.isSafeDistance)
                        {
                            MessageBox.Show("Cannot book a seat beside another person");
                            return;
                        }
        ...
        public void CreateSeats(object sender, System.EventArgs e)
        {
            ...
            try
            {
                maxRow = Convert.ToInt32(textRow.Text);
                maxCol = Convert.ToInt32(textCol.Text);
                dividerRow = textBox_RowDiv.Text;// 2,3,6 -> 10
                dividerCol = textBox_ColDiv.Text;
            }
            catch
            {
                labelMessage.Text = ("Invalid input entered");
                MessageBox.Show("Please use an int");
                return;
            }
        ...
        private void CreateSeatsSafeDistance(object sender, EventArgs e)
        {
            movie.SeatList = new SeatDoubleLinkedList();
            this.panelSeats.Controls.Clear();
            labelMessage.Text = ("Generated Seats in Safe Distance Mode!");
            if (textRow.Text == "" || textCol.Text == "")
            {
                return;
            }
            ...
            for (int row = 0; row < maxRow; row+=1)
            {
                ...
                {
                    Seat s1 = movie.SeatList.SearchByRowAndColumn(row+1, col+1);
                    Seat s2 = null;
                    if (col + 1 < maxCol)
                    {
                        s2 = movie.SeatList.SearchByRowAndColumn(row+1, col + 2);
                    }
                    if(row%2 == 0)
                    {
                        bool isWhite = false;
        ...
        private void ButtonReset_Click(object sender, EventArgs e)
        {
            ...
            enableSeats.Enabled = false;
            disableSeats.Enabled = false;
        ...
        private void ButtonEditorMode_Click(object sender, EventArgs e)
        {
            ...
            if (!isEditorMode)
            {
                ...
                enableSeats.Enabled = true;
                disableSeats.Enabled = true;
        ...
        private void ButtonExitSimulation_Click(object sender, EventArgs e)
        {
            DisableAllSeats();
            personBookingNum = -1; // int cannot be null, use a fake int, since 0 is personA
            personBooking = "";
            ButtonA.Enabled = false;
            ButtonA.Text = ("Person A booked finish!");
            ButtonB.Enabled = false;
            ButtonB.Text = ("Person B booked finish!");
            ButtonC.Enabled = false;
            ButtonC.Text = ("Person C booked finish!");
            ButtonD.Enabled = false;
            ButtonD.Text = ("Person D booked finish!");
        }
        ...
        {
            MessageBox.Show("Saved everything! Please choose a file to save into");
            Stream stream;
